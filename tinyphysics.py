...
class TinyPhysicsModel:
  ...
  def get_current_lataccel(self, sim_states: List[State], actions: List[float], past_preds: List[float]) -> float:
        ...
        actions = np.array(actions, dtype="object").reshape(-1, 1)
...
class TinyPhysicsSimulator:
  ...
  def control_stepNoAction(self, step_idx: int) -> None:
    if step_idx >= CONTROL_START_IDX:
  ...
  def get_state_target(self, step_idx: int) -> Tuple[List, float]:
      if step_idx < 0 or step_idx >= len(self.data):
          raise IndexError(f"step_idx {step_idx} is out-of-bounds for dataframe with length {len(self.data)}")
      
      if self.debug:
          print(f"get_state_target called with step_idx: {step_idx}")
          print(f"Data length: {len(self.data)}")

      state = self.data.iloc[step_idx]
      return [state['roll_lataccel'], state['v_ego'], state['a_ego']], state['target_lataccel']

  def step(self, action) -> None:
      if self.step_idx >= len(self.data):
          raise IndexError(f"step_idx {self.step_idx} is out-of-bounds for dataframe with length {len(self.data)}")
  ...
  def stepNoAction(self) -> None:
    state, target = self.get_state_target(self.step_idx)
    self.state_history.append(state)
    self.target_lataccel_history.append(target)
    self.control_stepNoAction(self.step_idx)
  ...
  if __name__ == "__main__":
      ...
      if data_path.is_file():
          cost, _, _ = rollout(data_path, args.controller, args.model_path, debug=args.debug)
          ...
          run_rollout_partial = partial(rollout, controller_type=args.controller, model_path=args.model_path, debug=False)
